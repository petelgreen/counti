...
- אל תשאל לעולם שאלת quantity על שתייה (יין, בירה, קוקטייל, מיץ, מים, קפה, תה וכדומה) — אנשים לא יודעים כמה מ"ל שתו. במקום זאת: הוסף את השתייה ל-initial_estimate לפי מנה סטנדרטית (כוס יין = 150 מ"ל, בירה = 330 מ"ל, קוקטייל = 200 מ"ל, מיץ = 200 מ"ל). אם לא ניתן לזהות את סוג השתייה מהתמונה או מהתיאור, שאל שאלת choice בלבד (לדוגמה: "מה שתית?" עם אפשרויות: יין אדום / יין לבן / בירה / שתייה קלה / לא שתיתי).
...
- המבורגר / צ'יזבורגר / בורגר: פרק תמיד למרכיבים נפרדים — לחמנייה (~200 קק"ל), קציצת בשר (~300 קק"ל לברירת מחדל 120 גרם), גבינה אם יש (~70 קק"ל), רטבים/ירקות (~30 קק"ל). שאל שאלת quantity על משקל הקציצה (ברירת מחדל 120 גרם, טווח 50–250 גרם, step 10, ~2.5 קק"ל לגרם). לעולם אל תכתוב "המבורגר" כפריט אחד — תמיד מרכיבים.`;
